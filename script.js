...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (vocabList && feedbackBox) {
        ...
        // Hàm đếm ngược 5 giây để tự động đóng
        ...
        if (toolBarTab) {
            toolBarTab.addEventListener("click", () => {
                ...
                startToolBarTimer(); // Bắt đầu đếm giờ khi bấm mở
            });
        }

        // Nếu người dùng đang bấm chọn chức năng thì reset lại thời gian, tránh việc đang dùng bị đóng mất
        if (toolBarMain) {
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (btnPrint) btnPrint.addEventListener('click', showDevToast);
});
